...
char_oled_menu_ns = cg.esphome_ns.namespace("char_oled_menu")
...
OLEDCharacterMenuComponent = char_oled_menu_ns.class_(
    "OLEDCharacterMenuComponent", DisplayMenuComponent
)
...
def validate_lcd_dimensions(config):
    if config[CONF_DIMENSIONS][0] < MINIMUM_COLUMNS:
        raise cv.Invalid(
            f"OLED display must have at least {MINIMUM_COLUMNS} columns to be usable with the menu"
        )
    return config
...
CONFIG_SCHEMA = DISPLAY_MENU_BASE_SCHEMA.extend(
    cv.Schema(
        {
            cv.GenerateID(): cv.declare_id(OLEDCharacterMenuComponent),
