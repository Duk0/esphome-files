...
class GPIOOLEDDisplay : public char_oled_base::OLEDDisplay {
 public:
  void set_writer(std::function<void(GPIOOLEDDisplay &)> &&writer) { this->writer_ = std::move(writer); }
  void setup() override;
  void set_data_pins(GPIOPin *d0, GPIOPin *d1, GPIOPin *d2, GPIOPin *d3) {
    ...
    this->data_pins_[3] = this->_busy_pin = d3;
  }
/*  
