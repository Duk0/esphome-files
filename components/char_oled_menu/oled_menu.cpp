...
static const char *const TAG = "char_oled_menu";

void OLEDCharacterMenuComponent::setup() {
...
float OLEDCharacterMenuComponent::get_setup_priority() const { return setup_priority::PROCESSOR - 1.0f; }

void OLEDCharacterMenuComponent::dump_config() {
  ESP_LOGCONFIG(TAG, "OLED Menu");
...
void OLEDCharacterMenuComponent::draw_item(const display_menu_base::MenuItem *item, uint8_t row, bool selected) {
